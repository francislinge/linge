...
def jinqixinxi_register(request):
    if request.method in ["POST", "GET"]:
        msg = {'code': normal_code, "msg": mes.normal_code}
        req_dict = request.session.get("req_dict")


        error = jinqixinxi.createbyreq(jinqixinxi, jinqixinxi, req_dict)
...
def jinqixinxi_login(request):
    if request.method in ["POST", "GET"]:
        msg = {'code': normal_code, "msg": mes.normal_code}
        req_dict = request.session.get("req_dict")
        datas = jinqixinxi.getbyparams(jinqixinxi, jinqixinxi, req_dict)
        if not datas:
        ...
        return Auth.authenticate(Auth, jinqixinxi, req_dict)


def jinqixinxi_logout(request):
    if request.method in ["POST", "GET"]:
        msg = {
            "msg": "登出成功",
            "code": 0
...
def jinqixinxi_resetPass(request):
    '''
    '''
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code}
        ...
        columns=  jinqixinxi.getallcolumn( jinqixinxi, jinqixinxi)

        try:
            __loginUserColumn__= jinqixinxi.__loginUserColumn__
        except:
        ...
        records=jinqixinxi.getbyparams(jinqixinxi, jinqixinxi, recordsParam)
        if len(records)<1:
            msg['code'] = 400
            msg['msg'] = '用户不存在'
            return JsonResponse(msg, encoder=CustomJsonEncoder)

        eval('''jinqixinxi.objects.filter({}='{}').update({}='{}')'''.format(username_str,username,password_str,init_pwd))
        
        return JsonResponse(msg, encoder=CustomJsonEncoder)



def jinqixinxi_session(request):
    '''
    '''
    if request.method in ["POST", "GET"]:
        ...
        msg['data']  = jinqixinxi.getbyparams(jinqixinxi, jinqixinxi, req_dict)[0]

        return JsonResponse(msg, encoder=CustomJsonEncoder)


def jinqixinxi_default(request):

    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code,"msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
        req_dict.update({"isdefault":"是"})
        data=jinqixinxi.getbyparams(jinqixinxi, jinqixinxi, req_dict)
        if len(data)>0:
            msg['data']  = data[0]
        else:
            msg['data']  = {}
...
def jinqixinxi_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        msg['data']['pageSize']  =jinqixinxi.page(jinqixinxi, jinqixinxi, req_dict, request)
        return JsonResponse(msg, encoder=CustomJsonEncoder)

def jinqixinxi_autoSort(request):
    '''
...
主要信息列表（如商品列表，新闻列表）中使用，显示最近点击的或最新添加的5条记录就行
    ...
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code,  "data":{"currPage":1,"totalPage":1,"total":1,"pageSize":10,"list":[]}}
        req_dict = request.session.get("req_dict")
        if "clicknum"  in jinqixinxi.getallcolumn(jinqixinxi,jinqixinxi):
            req_dict['sort']='clicknum'
        elif "browseduration"  in jinqixinxi.getallcolumn(jinqixinxi,jinqixinxi):
            req_dict['sort']='browseduration'
        else:
            req_dict['sort']='clicktime'
        req_dict['order']='desc'
        msg['data']['list'], msg['data']['currPage'], msg['data']['totalPage'], msg['data']['total'], \
        msg['data']['pageSize']  = jinqixinxi.page(jinqixinxi,jinqixinxi, req_dict)

        return JsonResponse(msg, encoder=CustomJsonEncoder)

#分类列表
def jinqixinxi_lists(request):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code,  "data":[]}
        msg['data'],_,_,_,_  = jinqixinxi.page(jinqixinxi, jinqixinxi, {})
        return JsonResponse(msg, encoder=CustomJsonEncoder)

def jinqixinxi_query(request):
    '''
    '''
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        try:
            query_result = jinqixinxi.objects.filter(**request.session.get("req_dict")).values()
...
def jinqixinxi_list(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        try:
            __foreEndList__=jinqixinxi.__foreEndList__
        except:
            __foreEndList__=None
        try:
            __foreEndListAuth__=jinqixinxi.__foreEndListAuth__
        except:
            __foreEndListAuth__=None

        #authSeparate
        try:
            __authSeparate__=jinqixinxi.__authSeparate__
        except:
        ...
        try:
            __authTables__=jinqixinxi.__authTables__
        except:
            __authTables__=None
        ...
        if jinqixinxi.__tablename__[:7]=="discuss":
            try:
                del req_dict['userid']
            except:
                pass
        ...
        q = Q()
        msg['data']['list'], msg['data']['currPage'], msg['data']['totalPage'], msg['data']['total'], \
        msg['data']['pageSize']  = jinqixinxi.page(jinqixinxi, jinqixinxi, req_dict, request, q)
        return JsonResponse(msg, encoder=CustomJsonEncoder)

def jinqixinxi_save(request):
    '''
    后台新增
    '''
    if request.method in ["POST", "GET"]:
        ...
        idOrErr= jinqixinxi.createbyreq(jinqixinxi,jinqixinxi, req_dict)
...
def jinqixinxi_thumbsup(request,id_):
    '''
    ...
    if request.method in ["POST", "GET"]:
        ...
        rets=jinqixinxi.getbyid(jinqixinxi,jinqixinxi,id_)
        ...
        error = jinqixinxi.updatebyparams(jinqixinxi,jinqixinxi, update_dict)
        if error!=None:
            msg['code'] = crud_error_code
            msg['msg'] = error
        return JsonResponse(msg, encoder=CustomJsonEncoder)
...
def jinqixinxi_info(request,id_):
    '''
    '''
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}

        data = jinqixinxi.getbyid(jinqixinxi,jinqixinxi, int(id_))
        if len(data)>0:
            msg['data']=data[0]
        ...
        #浏览点击次数
        try:
            __browseClick__= jinqixinxi.__browseClick__
        except:
            __browseClick__=None

        if __browseClick__=="是"  and  "clicknum"  in jinqixinxi.getallcolumn(jinqixinxi,jinqixinxi):
            try:
                clicknum=int(data[0].get("clicknum",0))+1
            except:
                clicknum=0+1
            click_dict={"id":int(id_),"clicknum":clicknum,"clicktime":datetime.datetime.now()}
            ret=jinqixinxi.updatebyparams(jinqixinxi,jinqixinxi,click_dict)
            if ret!=None:
                msg['code'] = crud_error_code
                msg['msg'] = ret
        return JsonResponse(msg, encoder=CustomJsonEncoder)

def jinqixinxi_detail(request,id_):
    '''
    '''
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
...
def jinqixinxi_update(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if 'clicktime' in req_dict.keys() and req_dict['clicktime']=="None":
            del req_dict['clicktime']
        if req_dict.get("mima") and "mima" not in jinqixinxi.getallcolumn(jinqixinxi,jinqixinxi) :
            del req_dict["mima"]
        if req_dict.get("password") and "password" not in jinqixinxi.getallcolumn(jinqixinxi,jinqixinxi) :
        ...
        error = jinqixinxi.updatebyparams(jinqixinxi, jinqixinxi, req_dict)
        if error!=None:
            msg['code'] = crud_error_code
            msg['msg'] = error
...
def jinqixinxi_delete(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        error=jinqixinxi.deletes(jinqixinxi,
            jinqixinxi,
             req_dict.get("ids")
        )
        if error!=None:
...
def jinqixinxi_vote(request,id_):
    '''
    ...
    if request.method in ["POST", "GET"]:
        ...
        for i in data:
            ...
            if votenum!=None:
                ...
                error=jinqixinxi.updatebyparams(jinqixinxi,jinqixinxi,params)
                if error!=None:
                    msg['code'] = crud_error_code
                    msg['msg'] = error
        return JsonResponse(msg)

def jinqixinxi_importExcel(request):
    if request.method in ["POST", "GET"]:
        ...
        if file_type in ['xlsx', 'xls']:
            ...
            try:
                for row in range(1, rows):
                    ...
                    jinqixinxi.createbyreq(jinqixinxi, jinqixinxi, req_dict)
...
def jinqixinxi_autoSort2(request):
...
def jinqixinxi_cleanse(request):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": {}}
        try:
            list, _, _, total,_ = jinqixinxi.page(jinqixinxi, jinqixinxi, {})
            ...
            jinqixinxi.deletes(jinqixinxi,jinqixinxi,ids=[i["id"] for i in list])
            batchList = []
            for dl in data_list:
                filtered_data = {k: v for k, v in dl.items() if v not in [None, '', float('nan')] and (not isinstance(v, float) or not math.isnan(v))}
                batchList.append(jinqixinxi(**filtered_data))
            jinqixinxi.objects.bulk_create(batchList)
        except Exception as e:
            msg["code"] = other_code
            msg["msg"] = e.__str__()
