	...
	<div class="main-content" :style='{"width":"100%","padding":"20px 20px 20px 30px","fontSize":"15px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"20px solid #fff","margin":"0 0px 0px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex"}' >
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">记录时间</label>
					...
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">省份</label>
						<el-input v-model="searchForm.shengfen" placeholder="省份" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">城市</label>
						<el-input v-model="searchForm.chengshi" placeholder="城市" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">县区</label>
						<el-input v-model="searchForm.juqu" placeholder="县区" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button class="add" v-if="isAuth('zaihaixinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('zaihaixinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						删除
					</el-button>

					<el-button class="btn18" v-if="isAuth('zaihaixinxi','导入')" type="success" @click="importHandler()">
						<span class="icon iconfont icon-daoru11" :style='{"margin":"0 2px","fontSize":"14px","color":"#939393","height":"34px"}'></span>
						导入
					</el-button>
					<el-button class="btn18" v-if="isAuth('zaihaixinxi','上传模板')" type="success" @click="importClcik">
						<span class="icon iconfont icon-shangchuan8" :style='{"margin":"0 2px","fontSize":"14px","color":"#939393","height":"34px"}'></span>
						上传模板
					</el-button>
					<el-button class="btn18" v-if="isAuth('zaihaixinxi','下载模板')" type="success" @click="download('upload/zaihaixinxi_template.xlsx')">
					...
					<el-button class="btn18" v-if="isAuth('zaihaixinxi','城市灾害统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#939393","height":"34px"}'></span>
						城市灾害统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('zaihaixinxi','山洪记录')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#939393","height":"34px"}'></span>
						山洪记录
					</el-button>
					<el-button class="btn18" v-if="isAuth('zaihaixinxi','降水量统计')" type="success" @click="chartDialog3()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#939393","height":"34px"}'></span>
						降水量统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('zaihaixinxi','人口密度')" type="success" @click="chartDialog4()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#939393","height":"34px"}'></span>
						人口密度
					</el-button>
				</el-row>
			...
			<div :style='{"border":"20px solid #fff","padding":"0","color":"#000","background":"#fff","borderWidth":"0 20px 20px","width":"100%","fontSize":"14px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="jiangshuiqiangdu"
						label="降水强度(mm/h)">
						<template slot-scope="scope">
							{{scope.row.jiangshuiqiangdu}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="qiwen"
					...
					<el-table-column :resizable='true' :sortable='true'  
						...
						label="地质构造">
						<template slot-scope="scope">
							{{scope.row.dizhigouzao}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="lishishanhongjilu"
						label="历史山洪记录">
						<template slot-scope="scope">
							{{scope.row.lishishanhongjilu}}
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('zaihaixinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 0px","fontSize":"12px","color":"#fff","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('zaihaixinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('zaihaixinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<el-dialog title="导入" :visible.sync="importVisiable" width="50%">
			<el-form ref="form" :model="form" label-width="80px">
				<el-form-item class="upload" label="文件" prop="excelFile"><excel-file-upload tip="点击上传直接导入excel文件" action="zaihaixinxi/importExcel" :limit="1" :fileUrls="importUrl" @change="importChange"></excel-file-upload>
				</el-form-item>
			</el-form>
		...
		<el-dialog title="上传模板" :visible.sync="importVis" width="50%">
			<el-form ref="form" :model="importForm" label-width="80px">
				<el-form-item class="upload" label="文件" prop="excelFile">
					<el-upload class="upload-demo" drag :action="$base.url + 'file/upload?type=zaihaixinxi_template'"
		...
		<el-dialog
			...
			width="800">
			<div id="lishishanhongjiluChart2" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="jiangshuiliangChart3" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="renkoumiduChart4" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
	...
	export default {
		data() {
			return {
				...
				boardBase: {"funnelNum":8,"lineNum":10,"gaugeNum":8,"barNum":10,"pieNum":10},
		...
		methods: {
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					...
					this.$http({
						url: "zaihaixinxi/group/chengshi",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							// 使用刚指定的配置项和数据显示图表。
							chengshiChart1.setOption(option);//根据窗口的大小变动图表
			...
			chartDialog2() {
				...
				this.$nextTick(()=>{
					...
					var lishishanhongjiluChart2 = echarts.init(document.getElementById("lishishanhongjiluChart2"),'macarons');
					this.$http({
						url: "zaihaixinxi/group/lishishanhongjilu",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							// 使用刚指定的配置项和数据显示图表。
							lishishanhongjiluChart2.setOption(option);
							//根据窗口的大小变动图表
							window.onresize = function() {
								lishishanhongjiluChart2.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			chartDialog3() {
				...
				this.$nextTick(()=>{
					...
					var jiangshuiliangChart3 = echarts.init(document.getElementById("jiangshuiliangChart3"),'macarons');
					this.$http({
						url: `zaihaixinxi/value/chengshi/jiangshuiliang`,
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							// 使用刚指定的配置项和数据显示图表。
							jiangshuiliangChart3.setOption(option);
							//根据窗口的大小变动图表
							window.onresize = function() {
								jiangshuiliangChart3.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			chartDialog4() {
				...
				this.$nextTick(()=>{
					...
					var renkoumiduChart4 = echarts.init(document.getElementById("renkoumiduChart4"),'macarons');
					this.$http({
						url: `zaihaixinxi/value/chengshi/renkoumidu`,
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							// 使用刚指定的配置项和数据显示图表。
							renkoumiduChart4.setOption(option);
							//根据窗口的大小变动图表
							window.onresize = function() {
								renkoumiduChart4.resize();
							};
						}else{
							this.$message({
								message: data.msg,
        ...
        if(this.searchForm.juqu!='' && this.searchForm.juqu!=undefined){
					params['juqu'] = '%' + this.searchForm.juqu + '%'
				}
				let user = JSON.parse(this.$storage.getObj('userForm'))
				this.$http({
					url: "zaihaixinxi/page",
					method: "get",
					params: params
				}).then(({ data }) => {
					if (data && data.code === 0) {
